...
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage[a4paper,width=170mm,top=18mm,bottom=21mm,includeheadfoot]{geometry}
\usepackage{booktabs}
\usepackage{array}
\usepackage{verbatim}
\usepackage{caption}
...
\title{ETH-Paid Internet Access\\ {\smaller \textbf{Master thesis}}}
...
\begin{abstract}
  \begin{center}
    ...
    licensing spectrum bands to states;
...
network, dnsmasq was used.

\vspace{0.35cm}

iptables\cite{iptables} was used as a firewall running
inside the Raspberry; by default, it was configured to
drop all packet
forwarding traffic from users connected to the wireless
domain and at the same time to redirect all
forwarding traffic to Raspberry's HTTP port where a
captive portal was deployed.
